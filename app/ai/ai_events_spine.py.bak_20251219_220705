...
# -*- coding: utf-8 -*-
"""
Flashback — AI Events Spine (disk-logging version, v2.8.0 Phase4 pending reconciliation hardened)

v2.8.0 Patch Summary (2025-12-18)
---------------------------------
CRITICAL: pending_setups reconciliation hardening.

Problem:
- pending_setups.json is keyed by setup trade_id (often "<account_label>:<client_id>")
- outcome_record trade_id can arrive as raw orderLinkId/orderId without prefix
- net effect: pending never matches, pending never clears, integrity check FAIL forever

Fix:
- When writing pending for setup_context, store alias keys:
    • trade_id (canonical)
    • client_trade_id (payload.extra / features)
    • source_trade_id (payload.extra / features)
    • unprefixed source (strip "<account_label>:")
- When processing outcome_record:
    • try direct lookup
    • else try alias lookup
    • once matched, pop ALL alias keys for that setup
"""
...
def _extract_setup_alias_keys(setup_event: Dict[str, Any]) -> Set[str]:
    ...
    if tid:
        ...
        # also store "unprefixed" source form if it looks like "<acct>:<id>"
        if ":" in tid:
...
def _find_pending_setup(pending: Dict[str, Any], trade_id: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    # alias scan (bounded by PEND_MAX_COUNT; acceptable for robustness)
...
def _remove_pending_setup(pending: Dict[str, Any], setup_event: Dict[str, Any]) -> None:
    """
    Remove ALL alias keys for this setup_event from pending.
    """
    try:
...
def publish_ai_event(event: Dict[str, Any]) -> None:
    ...
    if etype == "setup_context":
        ...
        if trade_id:
            try:
                pending = _load_pending()
                setup_evt, matched_key = _find_pending_setup(pending, str(trade_id))
            except Exception:
                setup_evt, matched_key = (None, None)
...
def main() -> None:
    log.info("AI Events Spine loop started (disk logger + heartbeat, v2.8.0).")
    while True:
        try:
