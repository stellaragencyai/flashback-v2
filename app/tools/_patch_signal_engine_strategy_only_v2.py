# -*- coding: utf-8 -*-
"""
Patch signal_engine.py:
- Add SIG_ALLOW_FALLBACK + SIG_FALLBACK_FANOUT env parsing
- Emit signals ONLY for strategies that match (no global fanout)
- Fallback runs only if allowed; optional fanout controlled by env
- (Optional) Add User-Agent header to Bybit kline fetch if missing
"""
...
def main() -> None:
    ...
    # 2) (Optional) 403 hardening: add a default User-Agent if fetch_recent_klines uses requests.get without headers
    # Only patch if the file does not already mention User-Agent anywhere.
    if ("User-Agent" not in s) and ("fetch_recent_klines" in s) and ("requests.get" in s):
    ...
    # 3) Replace the signal decision/emission block (the one in your dump ~0597..0752)
    # Start anchor: side/reason/debug init
    start_m = re.search(
    ...
    # End anchor: the generic append_signal_jsonl(...) call end (else branch)
    end_m = re.search(
        r'append_signal_jsonl\(\s*\n\s*symbol=symbol,\s*\n\s*side_text=side,\s*\n\s*tf_label=tf_label,\s*\n\s*bar_ts=bar_ts,\s*\n\s*price=last_close,\s*\n\s*reason=reason,\s*\n\s*debug=debug,\s*\n\s*sub_uid=None,\s*\n\s*strategy_name=None,\s*\n\s*\)\s*\n\s*\)\s*\n',
        s
    )
    if not end_m:
        die("PATCH FAILED: could not locate end of generic append_signal_jsonl block.")
...
'''
    ...
    print("OK: patched signal_engine.py (strategy-only emission + fallback gating).")
