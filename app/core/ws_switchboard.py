...
State format for positions_bus.json (v2, normalized)
----------------------------------------------------
{
  "version": 2,
...
State format for orderbook_bus.json (market_bus-compatible)
-----------------------------------------------------------
{
  "version": 1,
  "updated_ms": 1763752000123,
...
State format for trades_bus.json (market_bus-compatible)
--------------------------------------------------------
{
  "version": 1,
  "updated_ms": 1763752000123,
  "symbols": {
    "BTCUSDT": {
      "trades": [ {...}, {...}, ... ]   # capped list of recent trades per symbol
    },
    ...
  }
}
...
# Disable websocket-client internal tracing noise
...
def _load_api_creds(account_label: str) -> Tuple[Optional[str], Optional[str], str]:
    ...
    for key_env, sec_env in candidates:
        ...
        if key and sec:
            LOG.info(
                "[PRIVATE] Using Bybit API creds from env: %s / %s",
                key_env,
                sec_env,
            )
...
def _run_private_ws(
    ...
    def on_error(ws: websocket.WebSocketApp, error: Any) -> None:  # type: ignore
        err_str = str(error)
        LOG.error("[PRIVATE] WS error: %s", err_str)
    ...
    while not stop_event.is_set():
        try:
            ...
            # No client-side ping/pong; rely on server heartbeat
            ws.run_forever()
        except Exception as e:
...
def _run_public_ws(
    ...
    def on_error(ws: websocket.WebSocketApp, error: Any) -> None:  # type: ignore
        err_str = str(error)
        LOG.error("[PUBLIC] WS error: %s", err_str)
