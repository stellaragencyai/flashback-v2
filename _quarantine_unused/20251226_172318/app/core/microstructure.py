...
# -*- coding: utf-8 -*-
"""
Flashback — HFT Scalper (flashback05) v1.1
...
- Logs trade-open feature snapshots to app/state/features_trades.jsonl via feature_store.log_trade_open.
...
Safety / modes:
- HFT_FB05_LIVE=1  -> real trading (orders sent, brackets placed, mode="LIVE_CANARY").
- HFT_FB05_LIVE=0  -> PAPER / SIM mode:
      • No real orders.
      • Uses HFT_FB05_SIM_EQUITY as equity baseline if real equity <= 0.
      • Still runs full signal + guard checks.
      • Logs feature snapshots with mode="PAPER" for AI training.
"""
...
SUB_UID = "524637467"         # must match strategies.yaml for Sub5
...
# Simulated equity for PAPER / DRY-RUN mode when real equity <= 0
SIM_EQUITY = Decimal(os.getenv("HFT_FB05_SIM_EQUITY", "100"))
...
# ---------------------------------------------------------------------------
# Orders: entry + TP/SL bracket (LIVE only)
# ---------------------------------------------------------------------------
...
def place_hft_bracket(
    ...
    """
    Place 3 TP limit orders and 1 SL stop-market.
    ...
    body_sl = {
        "category": CATEGORY,
        "symbol": sym,
        "side": tp_side if is_long else "Sell" if not is_long else "Buy",
        "orderType": "Market",
        "timeInForce": "GTC",
        "reduceOnly": True,
...
def main_loop() -> None:
    log.info(
        "=== HFT Scalper v1.1 (%s) === live=%s, risk_pct=%s, sim_equity=%s",
        SUB_LABEL,
        HFT_LIVE,
        RISK_ALLOC_PCT,
        SIM_EQUITY,
    ...
    tg.info(
        ...
        f"Sim equity (if needed): {SIM_EQUITY} USDT"
    )

    while True:
        try:
            # Choose effective equity (real vs simulated)
            equity_real = get_equity_usdt()
            equity = equity_real
            if equity <= 0:
                if not HFT_LIVE:
                    equity = SIM_EQUITY
                    ...
                    log.warning("Equity <= 0 in LIVE mode, sleeping.")
                    time.sleep(LOOP_SLEEP_SEC)
                    continue
            else:
                equity_mode = "LIVE" if HFT_LIVE else "PAPER"
            ...
            for sym in HFT_SYMBOLS:
                ...
                # Approx position size and risk in USD (used for guard + logging)
                qty_est = (notional / mid).quantize(Decimal("0.0001"), rounding=ROUND_DOWN)
                if qty_est <= 0:
                    continue

                risk_usd = (R * SL_MULT * qty_est).quantize(Decimal("0.01"), rounding=ROUND_DOWN)
                if risk_usd <= 0:
                ...
                mode_str = "LIVE_CANARY" if HFT_LIVE else "PAPER"

                # ---------------- LIVE vs PAPER branches ----------------
                if HFT_LIVE:
                    ...
                    try:
                        side, qty = place_market_order(sym, direction, notional, mid)
                    except Exception as e:
                    ...
                    tp_levels, sl_level = place_hft_bracket(sym, direction, qty, mid, R)
                else:
                    # PAPER / SIM mode: no real orders, synthetic bracket only
                    side = "Buy" if direction == "LONG" else "Sell"
                    qty = qty_est
                    ...
                    for mult in TP_MULTS:
                        if is_long:
                            tp_levels.append((mid + mult * R).quantize(Decimal("0.0001"), rounding=ROUND_DOWN))
                        else:
                            tp_levels.append((mid - mult * R).quantize(Decimal("0.0001"), rounding=ROUND_DOWN))
                    if is_long:
                        sl_level = (mid - SL_MULT * R).quantize(Decimal("0.0001"), rounding=ROUND_DOWN)
                    else:
                        sl_level = (mid + SL_MULT * R).quantize(Decimal("0.0001"), rounding=ROUND_DOWN)
                ...
                # Feature store logging (both LIVE & PAPER)
                try:
                    log_trade_open(
                        sub_uid=SUB_UID,
                ...
                # Telegram summary
                title = "🟢 HFT ENTRY" if HFT_LIVE else "🧪 HFT PAPER ENTRY"
                msg_lines = [
                    title,
                    f"Sub: {SUB_LABEL}",
                    ...
                    f"SL (≈{SL_MULT}R): {sl_level}",
                    "TPs:",
                ]
                for i, tp in enumerate(tp_levels, start=1):
                    msg_lines.append(f"  • TP{i}: {tp}")
                eq_label = "real" if equity_mode != "SIM" else "sim"
                msg_lines.append(f"Equity: {equity} ({eq_label})")
                msg_lines.append(f"Guard: {guard_reason}")
                msg_lines.append(f"Reason: {signal['reason']}")
                if not HFT_LIVE:
                    msg_lines.append("Mode: PAPER (no real orders placed)")
