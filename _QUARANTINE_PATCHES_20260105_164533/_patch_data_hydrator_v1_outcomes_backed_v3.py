...
def main():
    ...
    if "OUTCOMES_V1 = STATE_DIR / \"ai_events\" / \"outcomes.v1.jsonl\"" in src:
        print("OK: patch already present (outcomes-backed stats).")
        return

    # Insert helper block after _now_ms() definition (right after it ends).
    # Find def _now_ms then insert after next blank line following it.
    ...
    # Now wire stats into hydrate_dashboard_rows:
    # After ops_accounts = ops.get("accounts", {}), load outcomes stats.
    ...
    # Replace total_trades/wins/losses derivation to prefer outcomes_stats when present.
    # We'll inject right after trades/perf/ai/risk extraction.
    ...
    # Replace the existing block of perf/trades/ai/risk extraction lines with injected block.
    out = out.replace(
        '        trades = ops_acct.get("trades", {})\n'
        '        perf = ops_acct.get("performance", {})\n'
        '        ai = ops_acct.get("ai", {})\n'
    ...
    # Now override total_trades/wins/losses/win_rate to use o_* if present else ops_snapshot.
    out = re.sub(
    ...
    # Finally, add PnL USD fields into the row so the UI has truthful metrics even if % is unknown.
    # Insert after win_rate_pct field.
